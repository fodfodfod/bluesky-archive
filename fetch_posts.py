    ...
    while not all_posts_fetched:
        ...
        for post in posts:
            ...
            if post in all_posts:
                all_posts_fetched = True
                logger.debug(f"Duplicate post found, ending fetch early for did: {did}, {util.did_to_username(did)}")
                break
            all_posts.append(post)
        cursor = data.get("cursor")
        logger.debug(f"Fetched {len(posts)} posts, total so far: {len(all_posts)}, did: {did}, {util.did_to_username(did)}")
